...
function rankingAlgy(username) {
	...
	//loop through users
	var main_classes = user_Ob.classes
	for(var us in users) {
		//Checks if college is the same == Main Filter
		//!!!!Add && not in rejects pile
		if( (us.college === user_Ob.college) && !(us.user in user_rejects) ) {
			...
			//>4
			if(common_classes > 4) {
				bin_gfour.push(us.user);
			};
			
			//Everything less or equal to 4
			switch(common_classes) {
				case 4:
					bin_four.push(us.user);
					break;
				case 3:
					bin_three.push(us.user);
					break;
				case 2:
					bin_two.push(us.user);
					break;
				case 1:
					bin_one.push(us.user);
					break;
				default:
					break;
			};
